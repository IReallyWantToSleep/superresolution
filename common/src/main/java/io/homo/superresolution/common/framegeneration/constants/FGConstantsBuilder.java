...
public final class FGConstantsBuilder {
	...
	public static FGCameraSnapshot build(
		CameraFrame current,
		...
		if (!resetHistory) {
			...
			requireInversePair(clipToPreviousClip, previousClipToClip, "clip history");
		}

		return new FGCameraSnapshot(
			current.frameIndex,
			toStreamlineMatrix(current.projection),
			toStreamlineMatrix(projectionInverse),
			toStreamlineMatrix(new Matrix4f()),
			toStreamlineMatrix(clipToPreviousClip),
			toStreamlineMatrix(previousClipToClip),
	...
	public static float[] toStreamlineMatrix(Matrix4fc matrix) {
	...
	static Matrix4f inverseChecked(Matrix4fc matrix, String name) {
		...
		requireInversePair(matrix, inverse, name);
		return inverse;
	}
	...
	private static void requireInversePair(Matrix4fc matrix, Matrix4fc inverse, String name) {
		Matrix4f product = new Matrix4f(matrix).mul(inverse);
		Matrix4f identity = new Matrix4f();
		if (!product.equals(identity, MATRIX_EPSILON)) {
