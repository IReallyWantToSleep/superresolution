...
#define NVG_MAX_FONT_AXES 3
#define NVG_FONT_AXIS_NAME_SIZE 16
...
extern "C"
{
    ...
    static void nvg__applyFontVariationAxes(NVGcontext *ctx, NVGstate *state)
    {
        ...
        if (state->fontId == FONS_INVALID)
            return;
        for (int i = 0; i < state->fontAxisCount && i < NVG_MAX_FONT_AXES; ++i)
        {
            if (state->fontAxisTag[i][0] != '\0')
            {
                fonsSetVariationAxis(ctx->fs, state->fontId, state->fontAxisTag[i], state->fontAxisValue[i]);
            }
        }
    }
    ...
    void nvgRestore(NVGcontext *ctx)
    {
        if (ctx->nstates <= 1)
            return;
        ctx->nstates--;
        // Re-apply font variation axes from the restored state
        nvg__applyFontVariationAxes(ctx, nvg__getState(ctx));
    }
    ...
    void nvgReset(NVGcontext *ctx)
    {
        ...
        state->fontId = 0;
        state->fontAxisCount = 0;
        for (int i = 0; i < NVG_MAX_FONT_AXES; ++i)
        {
    ...
    void nvgFontSetVariationAxis(NVGcontext *ctx, int font, const char *axisTag, float value)
    {
        ...
        // 如果设置的不是当前活动字体，直接传给后端即可，不需要更新当前 state
        ...
        // Update existing axis if present
        for (int i = 0; i < state->fontAxisCount && i < NVG_MAX_FONT_AXES; ++i)
        {
            if (strncmp(state->fontAxisTag[i], axisTag, NVG_FONT_AXIS_NAME_SIZE) == 0)
            {
                // 🌟 拦截无效更新
                if (state->fontAxisValue[i] == value)
                    return;
        ...
        // Add new axis if space available
        if (state->fontAxisCount < NVG_MAX_FONT_AXES)
        {
            int idx = state->fontAxisCount++;
            strncpy(state->fontAxisTag[idx], axisTag, NVG_FONT_AXIS_NAME_SIZE);
            state->fontAxisTag[idx][NVG_FONT_AXIS_NAME_SIZE] = '\0';
            state->fontAxisValue[idx] = value;
        }
        else
        {
            // No space: overwrite the last slot
            int idx = NVG_MAX_FONT_AXES - 1;
            strncpy(state->fontAxisTag[idx], axisTag, NVG_FONT_AXIS_NAME_SIZE);
            state->fontAxisTag[idx][NVG_FONT_AXIS_NAME_SIZE] = '\0';
            state->fontAxisValue[idx] = value;
        }
